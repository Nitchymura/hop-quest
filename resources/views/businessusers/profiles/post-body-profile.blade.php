...
<div class="card p-3 profile-post ">
        ...
        <div class="row align-items-center personal_space g-2">
                ...
                <button class="btn">
                    ...
                    @else
                        <a href="{{ route('profile.header', $post['user_id']) }}"><i
                                class="fa-solid fa-circle-user text-secondary profile-sm text-center"></i></a>
                    @endif
                </button>
            </div>
            ...
            <div class="col p-0">
                <a href="{{ route('profile.header', $post['user_id']) }}"
                    ...
                    <p class="username h5 my-auto">{{ $post['user']->name }}</p>
                </a>
            </div>
    ...
    @if ($post['type'] == 'businesses' || $post['type'] == 'quests' || $post['type'] == 'spots')
        ...
        <div class="d-flex align-items-center flex-wrap gap-3 mt-3 profile-stats">
            {{-- Likes --}}
            <div class="d-flex align-items-center gap-1 stat-item">
                @if ($post['is_liked'])
                    @php
                        $likeDeleteRoute = $post['type'] . '.like.delete';
                    @endphp
                    <form action="{{ route($likeDeleteRoute, $post['id']) }}" method="post" class="m-0">
                        @csrf
                        @method('DELETE')
                ...
                @else
                    @php
                        $likeStoreRoute = $post['type'] . '.like.store';
                    @endphp
                    <form action="{{ route($likeStoreRoute, $post['id']) }}" method="post" class="m-0">
                        @csrf
                        <button type="submit" class="btn p-0 border-0 bg-transparent">
                ...
                <button type="button" class="btn p-0 border-0 bg-transparent text-dark stat-count">
            ...
            {{-- Views --}}
            <div class="d-flex align-items-center gap-1 stat-item">
                <img src="{{ asset('images/chart.png') }}" alt="Views" class="stat-icon-img">
                <span class="stat-count">{{ $post['views_sum'] ?? 0 }}</span>
            </div>
        </div>
    @endif
