        ...
        <div class="page-container">
            ...
            <section class="main-image-section">
                <div class="main-image-wrapper mt-3">
                    <img src="{{ $business->main_image }}" alt="{{ $business->title }}"
                        class="card-img-top body-image main-image">
                    ...
                    <div class="main-overlay">
                        <div class="main-title">{{ $business->name }}</div>
                            ...
                            <div class="stat-item">
                                @if ($business->isLiked())
                                    <form action="{{ route('businesses.like.delete', $business->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn p-0 mt-3">
                        ...
                        <div class="main-dates">
                            @if ($business->term_start || $business->term_end)
                                <div class="event-dates">{{ $business->term_start }} - {{ $business->term_end }}</div>
                            @endif
                    ...
                    <div class="profile-left d-flex align-items-center justify-content-between">
                        ...
                        <div class="profile-main d-flex align-items-center">
                                ...
                                <button class="btn">
                                    <a href="{{ route('profile.header', $business->user->id) }}">
                                        @if ($business->user->avatar)
                                            <img src="{{ $business->user->avatar }}" alt=""
                            ...
                            <div class="col-auto profile-name ms-2 ">
                                <a href="{{ route('profile.header', $business->user->id) }}"
                                    class="text-decoration-none d-inline text-dark">
                                    {{ $business->user->name }}
                                </a>
                            </div>
                        ...
                        <div class="sns-icons d-flex align-items-center">
                            @if (!empty($business->user->instagram))
                                <a href="https://instagram.com/{{ $business->user->instagram }}"
                                    class="text-decoration-none d-flex align-items-center px-2" target="_blank"
                                    rel="noopener">
                                    <i class="fa-brands fa-instagram text-dark icon-md mx-1"></i>
                                </a>
                            @endif
                            @if (!empty($business->user->facebook))
                                <a href="https://facebook.com/{{ $business->user->facebook }}"
                                    class="text-decoration-none d-flex align-items-center px-2" target="_blank"
                                    rel="noopener">
                            ...
                            @if (!empty($business->user->x))
                                <a href="https://x.com/{{ $business->user->x }}"
                                    class="text-decoration-none d-flex align-items-center px-2" target="_blank"
                                    rel="noopener">
                            ...
                            @if (!empty($business->user->tiktok))
                                <a href="https://www.tiktok.com/@{{ $business - > user - > tiktok }}"
                                    class="text-decoration-none d-flex align-items-center px-2" target="_blank"
                                    rel="noopener">
                    ...
                    @if (count($business_photos) > 0)
                        <div class="promotion-carousel">
                            <div class="carousel-controls">
                                <button class="carousel-arrow prev" role="button"
                            ...
                            <div class="carousel-items-container">
                                @foreach ($business_photos as $index => $post)
                                    <div class="promotion-item {{ $index < 5 ? 'active' : '' }}">
                                        <div class="col">
                                            @if ($post->image)
                                                <img src="{{ $post->image }}" alt="Business Photo"
                                                    class="img-photo mb-2 mx-auto">
                                            @else
                                                No photos yet
                                            @endif
                                        </div>
            ...
            <div class="web-social card p-3 mt-3">
                <div class="web-social-content">

                    {{-- Website --}}
                    <div class="mb-3">
                        <div class="small text-muted mb-1">Official Website</div>
                        @if (!empty($business->website_url))
                            <a href="{{ $business->website_url }}" target="_blank"
                                class="fw-semibold text-decoration-none website-link">
                    ...
                    {{-- Social --}}
                    <div>
                        <div class="small text-muted mb-2">Social Media</div>

                        <div class="d-flex gap-3 align-items-center social-icons">
                                    ...
                                    @if ($hour->is_closed == 0)
                                        ...
                                        <td>
                                            @if (!empty($hour['opening_time']))
                                                {{ date('H:i', strtotime($hour['opening_time'])) }} -
                                                {{ date('H:i', strtotime($hour['closing_time'])) }}
                                            @else
                                                ー
                                            @endif
                                        </td>
                                ...
                                <div class="amenity-grid">
                                    @foreach ($category->businessInfos as $info)
                                        ...
                                        <div class="amenity-item me-auto">
                                            @if ($isValid)
                                                <i class="fa-solid fa-circle-check color-green ms-2"></i>
                                                {{-- ①チェックあり（緑） --}}
                                            @else
                                                <i class="fa-solid fa-circle-xmark color-red ms-2"></i>
                                                {{-- ②チェックなし（赤） --}}
                                            @endif
