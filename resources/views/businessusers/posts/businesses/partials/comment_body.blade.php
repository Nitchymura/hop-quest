        ...
        <div class="comment-container profile-post">
            <div class="comment-content">
                ...
                <div class="comment-header my-2 d-flex align-items-center">
                    ...
                    {{-- アバターリンク --}}
                    <a href="{{ route('profile.header', $comment->user_id) }}"
                    ...
                    <div class="d-flex align-items-center flex-wrap">
                        <a href="{{ route('profile.header', $comment->user_id) }}" class="text-decoration-none">
                            <span
                                class="username h6 mb-0 text-decoration-none text-dark"><strong>{{ $comment->user->name }}</strong></span>
                        </a>
                        {{-- 認証バッジ（任意） --}}
                            ...
                            <div class="comment-action-item">
                                <form action="{{ route('questcomment.toggleLike', $comment->id) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-sm shadow-none comment-like-btn">
                                        <i
                                            class="{{ $comment->BusinessCommentlikes->where('user_id', Auth::id())->isNotEmpty() ? 'fa-solid text-danger' : 'fa-regular' }} fa-heart"></i>
                                    </button>
                                </form>
